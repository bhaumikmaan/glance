...
(function () {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (message.type === "dashboard/init" && settingsPreview) {
      ...
      if (onboardingProvider && onboardingBaseUrl) {
        const provider = onboardingProvider.value || "bitbucket";
        onboardingBaseUrl.value =
          provider === "github"
            ? message.payload?.githubApiBaseUrl || ""
            : message.payload?.bitbucketBaseUrl || "";
      }
  ...
  onboardingProvider?.addEventListener("change", () => {
    ...
    if (!payload || !onboardingBaseUrl) return;
    onboardingBaseUrl.value =
      onboardingProvider.value === "github"
  ...
  function renderSnapshot(snapshot) {
    ...
    if (statusBadges && snapshot.badges) {
      statusBadges.innerHTML = `
        <span class="badge red">R ${snapshot.badges.red ?? 0}</span>
        <span class="badge yellow">Y ${snapshot.badges.yellow ?? 0}</span>
        <span class="badge green">G ${snapshot.badges.green ?? 0}</span>
      `;
    }
  ...
  function renderCurrentRepo(currentRepo) {
    ...
    if (currentCoreBranches) {
      currentCoreBranches.innerHTML = (currentRepo.coreBranches || [])
        .map(
          (branch) =>
            `<div>${escapeHtml(branch.name)} - Build Status: ${escapeHtml(capitalize(branch.status))}</div>`
        )
        .join("");
    }
    ...
    if (currentRecentBranches) {
      currentRecentBranches.innerHTML = (currentRepo.recentBranches || [])
        .slice(0, 12)
        .map(
          (branch) =>
            `<div>${escapeHtml(branch.name)} - ${branch.ageDays}d - ${escapeHtml(
              window.Glance.utils.formatTime(branch.lastCommitAt)
            )}</div>`
        )
        .join("");
    }
