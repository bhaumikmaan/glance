(function () {
  function initMyWork() {
      ...
      return items
        .map((item) => {
          ...
          const readinessClass =
            item.readiness === 'blocked' ? 'badge red' : item.readiness === 'pending' ? 'badge yellow' : 'badge green';
    ...
    function sortByReadiness(items, order) {
      const rank = order === 'my' ? { ready: 0, pending: 1, blocked: 2 } : { pending: 0, ready: 1, blocked: 2 };
