(function () {
  function initAuth(vscode) {
    ...
    function setProvider(provider, reveal = true) {
      ...
      if (onboardingBaseUrl) {
        ...
        onboardingBaseUrl.value =
          onboardingProvider === 'github' ? payload?.githubApiBaseUrl || '' : payload?.bitbucketBaseUrl || '';
      }
    ...
    function setProviderConnectionState(provider, isConfigured) {
      const isGitHub = provider === 'github';
      const disconnectedActions = isGitHub ? githubDisconnectedActions : bitbucketDisconnectedActions;
    ...
    function renderAuthStatus(payload) {
      ...
      if (githubStatus && payload?.github) {
        githubStatus.textContent = payload.github.configured ? 'Status: Configured' : 'Status: Not configured';
      ...
      if (bitbucketStatus && payload?.bitbucket) {
        bitbucketStatus.textContent = payload.bitbucket.configured ? 'Status: Configured' : 'Status: Not configured';
